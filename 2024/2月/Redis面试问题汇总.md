...
1. 针对Redis中热点数据的缓存怎么处理？
    ...
    * **具体逻辑**
        ...
        >
        > ex 参数表示设置键的过期时间，单位为秒。如果键到期后，则 Redis 会自动删除该键。
        >
        > 示例：
        >
        > ```shell
...
1. 更新Redis和MySQL时的数据一致性问题
    ...
    * 先更新数据库中的数据，后删除Redis中的缓存
        ...
        写请求A和读请求B同时过来，目前数据库的数据是1，写请求要将其修改为2。此时请求B先请求缓存，发现缓存不存在，就会将数据库中的1读出来。接着请求A先修改数据库中的数据为2，更新到数据库中，此时数据库是1，然后删除缓存。最后请求B将数据1缓存到缓存中，此时存在不一致。但是这种不一致只有一次，下次再有读请求过来就会将正确的数据更新到缓存中了，这里实现了**最终一致性**。

        但是上面说到的这种情况是很少见的，因为请求B从数据库中读取到1后，写入缓存的速度是远远快于请求A数据库的操作，所以请求A就可以在请求B之后删除错误的缓存，从而保证一致性。综上**先更新数据库中的数据，后删除Redis中的缓存是可以保证缓存最终一致性的**。
...
13. Redis的内存淘汰策略有哪些？
    ...
    * 第二类：进行数据淘汰
        * 设置了过期时间的键
            * **volatile-lru**：least recently used，淘汰最久没被访问的数据；
            * **volatile-lfu**：least frequently used，淘汰最不经常使用的数据；
            * **volatile-ttl**：time to live，淘汰最早过期的数据；
...
14. 为什么redis要将所有数据放到内存中
    * **性能优势**：内存的访问速度远远快于磁盘。将数据存储在内存中可以实现极快的读写操作，从而提供高性能的数据处理能力。这对于需要快速响应和处理大量数据的应用非常重要。
    * **简单性和高效性**：内存数据管理相对磁盘更为简单，不需要涉及复杂的磁盘 I/O 操作和文件系统。Redis 可以直接在内存中进行数据操作，减少了数据在磁盘和内存之间的转换开销，提高了系统的整体效率。
    * **弹性和可扩展性**：内存的扩展性相对较容易，可以通过增加内存资源来应对数据量的增长。相比于磁盘，扩展内存通常更简单和快速。

15. redis集群中为啥是16384 slot

    Redis Cluster模式下将存储数据的单元叫做slot-槽，在数据存储的时候，会利用键的哈希定位到该键值对存储的槽，计算公式是：`slot = CRC16(key) % 16384`。

    为什么是16384，为什么不是1000，1024，65536？

    * 16384 = 2¹⁴，二进制运算友好

      `% 16384` 本质是运算极快，实现简单稳定，Redis 是 C 写的，非常重视底层效率。

    * 如果槽位过多，节点之间发送心跳包，发送的心跳包过大，会浪费带宽。
    ...
    * 槽位越少，槽位的bitmap压缩比越高
      ...
      如果节点数不变，而哈希槽数量很多的话，bitmap的压缩率就很低。反之如果哈希槽位数量越少，则压缩比越高。
